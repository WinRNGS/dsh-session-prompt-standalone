...
import {
  ...
  rmSync, statSync, unwatchFile, watchFile, writeFileSync,
} from 'node:fs'
...
export interface Config { prompt?: string }
type InjectionRole = 'system' | 'user-context'
interface PromptLayer { enabled: boolean; prompt: string; role: InjectionRole; order: number }
interface WorkspaceLayer extends PromptLayer { path: string }
interface StoredSettings { version: 2; global: PromptLayer; workspaces: Record<string, WorkspaceLayer> }
...
type DiskSettingsState = {
  kind: 'missing' | 'valid' | 'invalid' | 'blocked'
  revision: string
  settings?: StoredSettings
  reason?: string
}

function validatePrompt(value: unknown, enabled = true): string {
  if (typeof value !== 'string') throw new PromptInputError(400, 'prompt must be a string')
  const prompt = value.trim()
  if (enabled && !prompt) throw new PromptInputError(400, 'enabled prompt cannot be empty')
  const bytes = Buffer.byteLength(prompt, 'utf8')
  if (bytes > MAX_PROMPT_BYTES) throw new PromptInputError(413, `prompt is too large (${bytes} bytes; limit ${MAX_PROMPT_BYTES})`)
...
function validateLayer(value: unknown, fallbackOrder: number): PromptLayer {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new PromptInputError(400, 'layer must be an object')
  const input = value as Record<string, unknown>
  const enabled = input.enabled === undefined ? true : input.enabled
  if (typeof enabled !== 'boolean') throw new PromptInputError(400, 'enabled must be a boolean')
  const role = input.role ?? 'system'
  if (role !== 'system' && role !== 'user-context') throw new PromptInputError(400, 'role must be "system" or "user-context"')
...
function defaultSettings(fallbackPrompt: string): StoredSettings {
 ...
 */
function recoverDisabledLayer(value: unknown, fallbackOrder: number): PromptLayer | undefined {
...
function parseStoredSettings(value: unknown, fallbackPrompt: string, warn: Warn): StoredSettings {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new Error('settings root must be an object')
  const input = value as Record<string, unknown>
  ...
  try {
    ...
    if (recovered !== undefined) settings.global = recovered
  ...
  if (input.workspaces !== undefined) {
    ...
    const workspaceEntries = Object.entries(input.workspaces as Record<string, unknown>)
    ...
    for (const [storedKey, raw] of workspaceEntries) {
      ...
      try {
        ...
        if (recovered !== undefined) settings.workspaces[key] = { ...recovered, path: resolvedPath }
...
function readDiskSettings(fallbackPrompt: string, warn: Warn): DiskSettingsState {
  try {
...
function serializeSettings(settings: StoredSettings): { text: string; revision: string } {
...
function saveSettings(settings: StoredSettings): string {
...
async function withSettingsLock<T>(action: () => T | Promise<T>): Promise<T> {
  mkdirSync(DSH_HOME, { recursive: true, mode: 0o700 })
...
function promptStats(prompt: string) {
...
function readBody(req: any): Promise<string> {
  return new Promise((resolveBody, reject) => {
    ...
    let bytes = 0
    let tooLarge = false
    req.on('data', (value: Buffer | Uint8Array | string) => {
...
function classifySource(sectionName: string): string {
  if (sectionName === GLOBAL_SYSTEM_NAME || sectionName === GLOBAL_CONTEXT_NAME) return '本插件 · 全局'
  if (sectionName === WORKSPACE_SYSTEM_NAME || sectionName === WORKSPACE_CONTEXT_NAME) return '本插件 · 工作区'
  if (sectionName === 'harness:identity') return 'DSH · Harness Identity'
  if (sectionName === 'deployment:persona') return 'DSH · Agent Preset / Persona'
...
function expectedRevision(req: any): string {
  const value = String(req.headers?.['if-match'] ?? '').trim()
  if (!value) throw new PromptInputError(428, 'If-Match revision is required; reload settings before saving')
  const normalized = value.startsWith('W/') ? value.slice(2).trim() : value
  if (normalized === '*') throw new PromptInputError(400, 'wildcard If-Match is not supported')
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  if (config.prompt?.trim()) {
    try { fallbackPrompt = validatePrompt(config.prompt) }
    catch (error) { ctx.logger.warn(`dsh-session-prompt: ignoring invalid config.prompt: ${error instanceof Error ? error.message : String(error)}`) }
  }
  const warn = (message: string): void => ctx.logger.warn(`dsh-session-prompt: ${message}`)
  ...
  if (initialDisk.kind === 'invalid') warn(`settings JSON is invalid; keeping defaults until a reviewed save: ${initialDisk.reason}`)
  if (writeBlockedReason !== undefined) warn(`settings writes are blocked: ${writeBlockedReason}`)
  type AgentLike = { id: unknown; ctx: Context; session: { header?: { cwd?: string } } }
  let globalDispose = () => {}
  const agentDisposers = new Map<AgentLike, () => void>()
  const workspaceFor = (agent: AgentLike): WorkspaceLayer | undefined => {
    const cwd = agent.session.header?.cwd
    return typeof cwd === 'string' && cwd ? current.workspaces[workspaceKey(cwd)] : undefined
  }
  const workspacePathFor = (agent: AgentLike): string | undefined => {
    const cwd = agent.session.header?.cwd
    return typeof cwd === 'string' && cwd ? resolve(cwd) : undefined
  }
  const layerText = (layer: PromptLayer | undefined, role: InjectionRole): string => layer?.enabled === true && layer.role === role ? layer.prompt : ''
  const installGlobal = (): (() => void) => combineDisposers([
  ...
  const installAgent = (agent: AgentLike): (() => void) => {
  ...
  const rebuildRegistrations = (next: StoredSettings): void => {
    ...
    const installAll = () => {
      ...
      for (const agent of agents) agentDisposers.set(agent, installAgent(agent))
    }
    clear(); current = next
    try { installAll() } catch (error) { clear(); current = previous; installAll(); throw error }
  }
  const adoptDiskState = (disk: DiskSettingsState, source: string): void => {
    if (disk.revision === currentRevision) return
  ...
  const refreshFromDisk = (source: string): void => {
    adoptDiskState(readDiskSettings(fallbackPrompt, warn), source)
  }

  const commitSettings = async (
    expectedRevision: string | undefined,
    update: (latest: StoredSettings) => StoredSettings,
  ): Promise<StoredSettings> => withSettingsLock(async () => {
  ...
  ctx.effect(() => {
    globalDispose = installGlobal()
    for (const rawAgent of (ctx as any).agents.list()) {
      const agent = rawAgent as AgentLike
      agentDisposers.set(agent, installAgent(agent))
    }
    return () => { globalDispose(); for (const dispose of agentDisposers.values()) dispose(); agentDisposers.clear() }
  }, 'dsh-session-prompt: prompt layers')
  ctx.on('agent/created', ({ agent }: any) => { if (!agentDisposers.has(agent)) agentDisposers.set(agent, installAgent(agent)) })
  ctx.on('agent/disposed', ({ agent }: any) => { agentDisposers.get(agent)?.(); agentDisposers.delete(agent) })
  ...
  ctx.effect(() => {
    ...
    const onWatch = (): void => {
      ...
      reloadTimer = setTimeout(() => {
        ...
        try { refreshFromDisk('file watcher') }
        catch (error) { warn(`file watcher reload failed: ${error instanceof Error ? error.message : String(error)}`) }
      }, 100)
    }
  ...
  ctx.on('tools/pre-execute', async (exec, next): Promise<PreToolDecision> => {
    ...
    if (downstream.kind !== 'allow' || exec.name !== 'dsh_session_prompt_set') return downstream
  ...
  ctx.effect(() => (ctx as any).tools.register({
    ...
    async execute(args: unknown) {
      if (args === null || typeof args !== 'object' || Array.isArray(args)) return 'ERROR: arguments must contain prompt'
      try {
        const prompt = validatePrompt((args as { prompt?: unknown }).prompt)
  ...
  ;(ctx as any).inject(['webServer'], (webCtx: any) => {
    webCtx.effect(() => webCtx.webServer.register({
      ...
      handler: async (req: any, res: any) => {
        const send = (status: number, body: unknown, responseRevision = currentRevision): void => {
        ...
        try {
          ...
          if (fetchSite && fetchSite !== 'same-origin' && fetchSite !== 'none') return send(403, { ok: false, error: 'cross-site requests are not allowed' })
          ...
          if (req.method === 'GET') refreshFromDisk('API read')
          const agent = sessionId ? (ctx as any).agents.get(sessionId) as AgentLike | undefined : undefined
          if (scope === 'workspace' && agent === undefined) return send(404, { ok: false, error: 'current session is not available; open or resume the session first' })
          const workspace = agent ? workspacePathFor(agent) : undefined
          const emptyWorkspace: PromptLayer = { enabled: false, prompt: '', role: 'system', order: 20 }
          const selected = scope === 'global' ? current.global : (workspaceFor(agent!) ?? emptyWorkspace)
          const preview = async () => {
            if (agent === undefined) return null
            try {
              const assembly = await ctx.systemPrompt.assemble({ agent, scope: agent } as any)
          ...
          if (req.method === 'POST') {
            ...
            if (mediaType !== 'application/json') return send(415, { ok: false, error: 'content type must be application/json' })
            const declaredLength = Number(req.headers?.['content-length'] ?? 0)
            if (Number.isFinite(declaredLength) && declaredLength > MAX_REQUEST_BYTES) return send(413, { ok: false, error: `request body exceeds ${MAX_REQUEST_BYTES} bytes` })
            const body = JSON.parse(await readBody(req))
            const layer = validateLayer(body?.layer, scope === 'global' ? 10 : 20)
            await commitSettings(expectedRevision(req), (latest) => {
              const next: StoredSettings = { ...latest, workspaces: { ...latest.workspaces } }
              if (scope === 'global') next.global = layer
              else next.workspaces[workspaceKey(workspace!)] = { ...layer, path: workspace! }
              return next
            })
            return send(200, {
          ...
          if (req.method === 'DELETE') {
            ...
            await commitSettings(expectedRevision(req), (latest) => {
              const next: StoredSettings = { ...latest, workspaces: { ...latest.workspaces } }
              if (scope === 'global') { layer = defaultSettings(fallbackPrompt).global; next.global = layer }
              else { delete next.workspaces[workspaceKey(workspace!)]; layer = emptyWorkspace }
              return next
            })
            return send(200, {
          ...
          if (error instanceof PromptInputError) return send(error.status, { ok: false, error: error.message, revision: currentRevision })
          if (error instanceof SyntaxError) return send(400, { ok: false, error: 'request body must be valid JSON' })
